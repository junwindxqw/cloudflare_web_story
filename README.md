...
- **📖 多格式阅读** — 支持 **EPUB / MOBI / AZW3 / FB2 / CBZ**（foliate-js 引擎，含分页、目录、双页排版）与 **PDF**（pdf.js 流式加载、缩放），**TXT** 自研分页引擎并自动识别 **UTF-8 / GBK / GB18030 编码**，中文小说无乱码
...
- **🎨 阅读体验** — 白色 / 羊皮纸 / 夜间三种阅读背景，字号调节，目录抽屉，进度滑杆快捷跳转
...
| 阅读 | 点击书架封面；有进度的书自动续读 |
| 翻页 | 点击左右 1/3 区域、左右方向键、触摸滑动；PDF 为滚动模式 |
| 工具栏 | 点击屏幕中央或滚动 PDF 呼出 / 隐藏 |
| 目录 | 顶栏 ☰；EPUB / TXT（自动识别章节标题）/ MOBI 均支持 |
| 阅读背景 / 字号 | 顶栏 ⚙：白色、羊皮纸、夜间三种背景 + 字号调节（PDF 为缩放） |

...
- **📖 多格式阅读** — 支持 **EPUB / MOBI / AZW3 / FB2 / CBZ**（foliate-js 引擎，含分页、目录、双页排版）与 **PDF**（pdf.js 流式加载、缩放、夜间反色），**TXT** 自研引擎并自动识别 **UTF-8 / GBK / GB18030 编码**，中文小说无乱码
...
- **🏷️ 自动分类** — 上传时按书名/作者启发式自动归入小说 / 科幻 / 计算机 / 历史 / 工具书 / 散文 / 杂志 等题材；顶部 chip 横条按分类筛选；菜单可手动改分类；一键重分类对全书架重新跑一遍
- **📱 双端自适应** — 响应式布局：手机网格、桌面多列、触摸滑动翻页、点击呼出工具栏、深浅色主题跟随系统
- **🎨 阅读体验** — **左右翻页 / 上下滚动** 两种阅读方式随时切换，白色 / 羊皮纸 / 夜间三种阅读背景，**字号 / 行距 / 字体（默认 / 宋体 / 黑体）** 三维度调节，目录与设置均为底部弹层，进度滑杆快捷跳转，**屏幕常亮** 防止夜间熄屏
- **⌨️ 键盘翻页** — `←` / `→` / `PageUp` / `PageDown` / `Space` 翻页，`Home` / `End` 跳到首/末页，点击左右 1/3 区域翻页、中央呼出工具栏
- **📲 手机端细节** — 登录页 input 16px 防 iOS 缩放、密码可见切换、验证码全宽按钮；书架顶栏折叠式搜索、菜单按钮常显、长按 500ms 防误触；阅读器顶/底栏触控热区加大、字号自适应（手机默认 20、桌面 18）、**点击屏幕中间切换底部菜单显隐、10 秒无操作自动隐藏**
...
| 阅读 | 点击书架封面；有进度的书自动续读 |
| 翻页 | 点击左右 1/3 区域、左右方向键、`PageUp`/`PageDown`/`Space`、触摸滑动（翻页模式）；`Home`/`End` 跳到首末页 |
| 阅读方式 | 底部工具栏「设置」→ 阅读方式：**左右翻页**（分栏横滑）或**上下滚动**（EPUB / TXT 支持，进度无缝保留） |
| 工具栏 | 点击屏幕中央呼出 / 隐藏；10 秒无操作自动隐藏 |
| 目录 | 底部工具栏 ☰；EPUB / TXT（自动识别章节标题）/ MOBI 均支持 |
| 阅读背景 / 字号 / 行距 / 字体 | 底部工具栏 ⚙：白色、羊皮纸、夜间三种背景 + 字号步进器 + 行距（紧凑 / 标准 / 宽松）+ 字体（默认 / 宋体 / 黑体）+ 屏幕常亮（PDF 自动用缩放替代字号行距字体，且恒为上下滚动） |
